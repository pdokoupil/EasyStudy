...
def calc_ratings(df_interaction, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, n_iterations):
...
def calc_beta_supports(df_interaction, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, df_weights, n_iterations):
    ...
    print(f"Num interactions = {df_interaction.shape}")
    for _, row in df_interaction.iterrows():
        d = json.loads(row.data)
        participation = row.participation
            ...
            if real != [int(x["movie_idx"]) for x in simulated_beta_result]: #, f"{real} != {[int(x['movie_idx']) for x in simulated_beta_result]}"
                ...
                for idx, m_index in enumerate(real):
                    ...
                    simulated_beta_result.append({
                        "movie_idx": m_index,
                        "support": {
                            "relevance": supports[0, 0, m_index],
                            "diversity": supports[1, 0, m_index],
                            "novelty": supports[2, 0, m_index]
                        },
                        "rank": idx
                    })
...
def calc_adjusted_supports(df_interaction, df_elicitation, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx):
    ...
    for _, row in df_interaction.iterrows():
        ...
        for idx, selected_info in enumerate(all_selected):
            ...
            supports[2, :, :] = wrapper.normalizations[2](supports[2].reshape(-1, 1)).reshape((supports.shape[1], -1)) * wrapper.discount_sequences[2][idx]
            ### End of support calculation ###
            m_index = selected_info["movie_idx"]
            result.append({
            ...
            new_row = [
                ...
                supports[0, 0, m_index],
                supports[1, 0, m_index],
                supports[2, 0, m_index],
...
def calc_adjusted_supports_2(df_interaction, df_elicitation, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, n_iterations):
    df_supports = pd.DataFrame(columns=[
        "userId",
        "movieId",
        "relevanceSupport",
...
def show_differences(df_interaction, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, df_selections, n_iterations):
...
if __name__ == "__main__":
    ...
    df_interaction = df_interaction[(df_interaction.interaction_type == "iteration-ended") & (df_interaction.iteration == args.n_iterations)]
    ...
    if args.gen_ratings:
        ...
        calc_ratings(df_interaction, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, args.n_iterations).to_csv("./ratings.csv", index=False)
    ...
    if args.gen_beta_supports:
        ...
        print(df_weights)
        print("Calc beta")
        calc_beta_supports(df_interaction, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, df_weights, args.n_iterations).to_csv("./beta_supports.csv", index=False)
    ...
    #calc_adjusted_supports_2(df_interaction, df_elicitation, df_elicitation_selections, df_impressions, df_impressions_beta, movie_title_to_idx, args.n_iterations).to_csv("./tmp_2.csv", index=False)
