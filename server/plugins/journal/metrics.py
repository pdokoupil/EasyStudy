...
class binomial_diversity:
    def __init__(self, all_categories, get_item_categories, rating_matrix, alpha):
    ...
    def __call__(self, rec_list):
        return self._coverage(rec_list) * self._non_red(rec_list)
    ...
    def _p_g_1(self, g):
        ...
        denom = self.rating_matrix.astype(bool).sum()
        nom = 0.0
    ...
    def _category_redundancy(self, g, k_g, N):
        ...
        for _ in range(1, k_g):
            ...
            # P(x_g = l) / (1 - P(X_g = 0))
            p = self._p_g(g)
            s += (self._binomial_probability(N, k_g, p) / (1.0 - self._binomial_probability(N, 0, p)))

        return 1.0 - s
    
    def _non_red(self, rec_list):
        rec_list_categories = self._get_list_categories(rec_list)
